...
export interface AvailabilityContextValue {
  ...
  setEventName: (date: Date, name: string) => void;
...
interface AvailabilityProviderProps {
  children: ReactNode;
  initialBlockedDates?: Map<string, BlockedDate>;  // For public calendar
  readOnly?: boolean;  // Disable localStorage sync for public view
}
...
export function AvailabilityProvider({
   ...
   * - Uses initialBlockedDates if provided (public calendar)
  ...
  /**
   * Saving state during localStorage write
   *
   * Brief state (300ms) to show visual feedback during save.
   */
   ...
   * - Triggers visual feedback (isSaving + lastSaved)
   *
  ...
  /**
   * Set event name for a blocked date
   *
   * Updates the eventName field for an existing blocked date.
   * Only works if the date is already blocked.
   *
   * @param date - Date to update
   * @param name - Event name/description (max 40 characters)
   *
   * @example
   * ```typescript
   * setEventName(new Date('2026-01-15'), 'Team Meeting');
   * // Adds "Team Meeting" to blocked date
   * ```
   */
  const setEventName = useCallback((date: Date, name: string) => {
    ...
    setBlockedDates((prev) => {
      ...
      // Only update if date is already blocked
      if (existing) {
        newMap.set(key, {
          ...existing,
          eventName: name.trim() || undefined, // Remove if empty
        });
      }
